...
- Width is fixed, height should be fixed as well. Baasically, make sure A4 size page constraint as I want to get book feel, and take prints. Maybe, we can add option for user to choose A3 and other page views in settings. (Helps for both printouts, and big screen)
...
- The first image could be bigger
- Only standalone images (if card level type is image) should be at the center of the page. Not the images that belong to Question / Answer or any other type.
...
- I don't think we need to keep the card in one page. When I add stories, one card could span multiple pages.
...
- We can break the content at any line to fill the gap. But, keep the excerpt and q/a ID along with the text content. I.e., don't pull them to fill the gap.
...
A: Not worried about card split. It can continue to next page.
...
- Jump to page instead of spread as a user.
